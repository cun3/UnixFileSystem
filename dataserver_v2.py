...
class SimpleHT:
  ...
  #retrun list of all keys
  def list_contents(self):
    print 'in list_contents'
    return self.data.keys()

  #corrupt value for given key
  def corrupt(self,key):
    self.check()
    end = datetime.now() + timedelta(seconds = 6000)
    print 'before corrupting',self.data[key.data]
    value=Binary(pickle.dumps('junk'))
    self.data[key.data] = ((value.data), end)
    print 'key value corrupted', self.data[key.data]
    return True
   
  #terminate server requests servicing
  def terminate(self):
    #global quit
    self.quit=True
    print 'terminated', quit
    return 1

  def fill(self,error_port):
    keys=[]
    keys=self.data.keys()
    print keys
    rpc = xmlrpclib.Server("http://127.0.0.1:"+error_port)
    for key in keys:
	res = self.get(Binary(key))
	print res
	val = pickle.loads(res["value"].data)	
	print val
	#rpc.put(Binary(key), self.data[key], 6000)
	rpc.put(Binary(key), Binary(pickle.dumps(val)), 6000)
    return True	
     

  #Check if key is present
  def check_key(self,key):
    print "key to check is",key
    key=key.data
    if key in self.data:
...
def main():
  ...
  if "--test" in ol:
    ...
    unittest.main()
    return
  print 'in main functionof port',port
  serve(port)
...
def serve(port):
  ...
  file_server.register_function(sht.check_key)
  ...
  file_server.register_function(sht.fill)  
  #global quit
  print 'in serve_port'
  print quit
  print 'nidhi'
